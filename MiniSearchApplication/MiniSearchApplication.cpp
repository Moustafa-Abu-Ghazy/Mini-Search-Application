...
void setup_filenames(int& number_of_files, string*& filenames) {
	...
	cout << "==" << "========================================" << "==" << endl;
	cout << "==" << " Enter Number Of Files: ";		cin >> number_of_files;
	while (!cin.good()) {
	...
	for (int i = 0; i < number_of_files; i++) { cout << "==" << i + 1 << "- ";	cin >> filenames[i]; }
	cout << "==" << "========================================" << "==" << endl;
}
...
void menu(int& choice, string& keyword, string& replace_of_keyword, int& number_of_files, string*& filenames) {
	...
	while (true) {
		cout << "==" << " ";    cin >> choice;
		if (cin.good()) {
			if (choice == 1 || choice == 2 || choice == 3) { cout << "==" << " Enter The Keyword: ";     cin >> keyword; }
			else if (choice == 4) {
				setup_filenames(number_of_files, filenames);
				return menu(choice, keyword, replace_of_keyword, number_of_files, filenames);
			}
			else if (choice == 5) { exit(0); }
			else { cout << "==" << " Please Choose A Correct Choice         " << "==" << endl;    continue; }
			if (choice == 2) { cout << "==" << " Replace It With:   ";      cin >> replace_of_keyword; }
		}
		else {
			//something went wrong, we reset the buffer's state to good
			cin.clear();
